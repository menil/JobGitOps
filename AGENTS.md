...
- **Self-reply Guard**: The assistant (`src/gitemployed/cli/respond.py`) skips comments authored by bots (`user.type == "Bot"`) or logins in `AGENT_BOT_LOGINS`, and any comment containing either automation marker: `<!-- gitemployed:status-update -->` (the conversational status-update confirmation) or `<!-- gitemployed:gmail-notice -->` (the Gmail-sync ambiguous-match heads-up comment, which is posted with a real-user-like token and would otherwise re-trigger `respond-issue.yml`). Never remove or weaken these guards — they are the only thing preventing infinite comment→respond→comment loops.
- **`gmail-sync-state` orphan branch**: like the `applications/*` branches, `gmail-sync-state` is automation-owned — `gmail_sync.py` alone reads and commits `data/gmail-state.json` (the Gmail de-dup cursor) on it. Never manually edit that file or merge this branch into `main`; it shares no history with `main` by design.
- **Tool Loop**: `assistant.run_agent` is a capped tool-calling loop over `web_search` / `fetch_url` only (see `src/gitemployed/web.py`). Identical tool calls within one run are memoized; web-tool errors are returned to the model as tool results, never raised. Side effects run only from the model's structured action JSON — web content is *data*, not instructions.
- **Projects V2 Sync Ownership & Loop-Safety**: Direct column and closure updates are performed in `respond.py` and `project_sync.py` to minimize latency and ensure consistency with terminal statuses (`CLOSURE_LABELS`), while `status-transition.yml` and `src/gitemployed/cli/project_sync.py backfill` serve as eventual consistency owners. The forward (label → column) and reverse (column → label, `project-status-sync.yml`) directions converge because re-setting an already-set column emits no `edited` event and the reverse script no-ops when the target label is already present with no stale siblings. `src/gitemployed/cli/project_sync.py` never re-adds the `triage-pending` label (its `REVERSE_SYNC_STATUSES` excludes Triage Pending), so dragging a card back cannot re-trigger `triage-issue.yml`. Do not weaken these guards — they prevent an infinite label↔column feedback loop.
