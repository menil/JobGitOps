...
- **Self-reply Guard**: The assistant (`src/jobgitops/cli/respond.py`) skips comments authored by bots (`user.type == "Bot"`) or logins in `AGENT_BOT_LOGINS`, and any comment containing either automation marker: `<!-- jobgitops:status-update -->` (the conversational status-update confirmation) or `<!-- jobgitops:gmail-notice -->` (the Gmail-sync ambiguous-match heads-up comment, which is posted with a real-user-like token and would otherwise re-trigger `respond-issue.yml`). Never remove or weaken these guards — they are the only thing preventing infinite comment→respond→comment loops.
...
- **Projects V2 Sync Ownership & Loop-Safety**: Direct column and closure updates are performed in `respond.py` and `project_sync.py` to minimize latency and ensure consistency with terminal statuses (`CLOSURE_LABELS`), while `status-transition.yml` and `src/jobgitops/cli/project_sync.py backfill` serve as eventual consistency owners. The forward (label → column) and reverse (column → label, `project-status-sync.yml`) directions converge because re-setting an already-set column emits no `edited` event and the reverse script no-ops when the target label is already present with no stale siblings. `src/jobgitops/cli/project_sync.py` never re-adds the `triage-pending` label (its `REVERSE_SYNC_STATUSES` excludes Triage Pending), so dragging a card back cannot re-trigger `triage-issue.yml`. Do not weaken these guards — they prevent an infinite label↔column feedback loop.
