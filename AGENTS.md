...
- **Projects V2 Sync Ownership & Loop-Safety**: `status-transition.yml` and `src/project_sync.py backfill` are the only callers of `update_project_status`; the responder's `status_update` action adds the label and posts a confirmation, never moving the column itself. The forward (label → column) and reverse (column → label, `project-status-sync.yml`) directions converge because re-setting an already-set column emits no `edited` event and the reverse script no-ops when the target label is already present with no stale siblings. `src/project_sync.py` never re-adds the `triage-pending` label (its `REVERSE_SYNC_STATUSES` excludes Triage Pending), so dragging a card back cannot re-trigger `triage-issue.yml`. Do not weaken these guards — they prevent an infinite label↔column feedback loop.
