...
2. Download that tag's tarball, extract **the shell-plane files only**: `.github/labels.yml` and the runtime-core workflows (same allowlist as §3). Never copies maintainer workflows (`build-runner.yml`, `ci.yml`, `pr-review.yml`).
