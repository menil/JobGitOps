...
def main() -> None:
    ...
    _transition(gh_client, context, LABEL_TO_STATUS[context.label])
...
def _resolve_context(args: argparse.Namespace) -> TransitionContext:
    """Resolve token, repository, issue, and label from CLI args and event payload.
    ...
    if event_path:
        ...
        try:
            ...
            # Label name can be resolved from the webhook payload
...
def _transition(
    gh_client: GitHubClient, context: TransitionContext, status: str
) -> None:
    """Move the issue's Projects V2 column to the given status."""
    ...
    try:
        ...
        logger.info("Successfully updated status to %s.", status)
    except Exception as e:
