# gitemployed-installer

Interactive bootstrap installer for
[GitEmployed](https://github.com/menil/GitEmployed) — a serverless, GitOps-driven
...
1. Creating a new private GitHub repository from the latest GitEmployed release
...
See the [main repository](https://github.com/menil/GitEmployed) for the full
daily workflow, configuration reference, and architecture.

## CLI options

```
Usage: gitemployed-installer [options] [repo-name]
```
...
| `--tag <ref>` | Install from a specific GitEmployed tag, branch, or commit |
| `--token <token>` | GitHub Personal Access Token (PAT) |

## Non-interactive example

```bash
npx gitemployed-installer my-job-search \
  --yes --provider gemini --gemini-key "$GEMINI_API_KEY"
```

## Pinning a template version

By default the installer uses the latest GitEmployed release. To install a
specific version:

```bash
npx gitemployed-installer my-job-search --tag v1.2.3
```
