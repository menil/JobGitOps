...
from gitemployed.cli.project_sync import main
...
def in_memory_event(event_data=None, json_side_effect=None):
    ...
    with (
        patch("gitemployed.cli.project_sync.pathlib.Path.open"),
        patch(
            "gitemployed.cli.project_sync.json.load",
...
def mock_load_settings(mock_settings) -> MagicMock:
    ...
    with patch("gitemployed.cli.project_sync.load_settings") as mocked:
...
def test_changed_status_unit(event, item, status_field_name, expected) -> None:
    ...
    from gitemployed.cli.project_sync import _changed_status
